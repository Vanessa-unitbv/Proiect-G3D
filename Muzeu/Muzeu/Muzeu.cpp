...
public:
    ...
    void ProcessKeyBoard(float deltaTime, EMovement movement)
    {
        switch (movement)
        {
        case EForward:
            this->position += (deltaTime * cameraSpeedFactor) * forward;
            break;
        case EBackward:
            this->position -= (deltaTime * cameraSpeedFactor) * forward;
            break;
        case ERight:
            this->position += (deltaTime * cameraSpeedFactor) * right;
            break;
        case ELeft:
            this->position -= (deltaTime * cameraSpeedFactor) * right;
            break;
        case EUp:
            this->position += (deltaTime * cameraSpeedFactor) * up;
            break;
        case EDown:
            this->position -= (deltaTime * cameraSpeedFactor) * up;
            break;
...
void Initialize()
{
    ...
    // Create camera
    pCamera = new Camera(SCR_WIDTH, SCR_HEIGHT, glm::vec3(0.5, 0.5, 50));
}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
        pCamera->ProcessKeyBoard((float)deltaTime, EMovement::EForward);

    }
    if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) {
        pCamera->ProcessKeyBoard((float)deltaTime, EMovement::EBackward);
    }
    if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) {
        pCamera->ProcessKeyBoard((float)deltaTime, EMovement::ELeft);
    }
    if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) {
        pCamera->ProcessKeyBoard((float)deltaTime, EMovement::ERight);
    }
    if (glfwGetKey(window, GLFW_KEY_PAGE_UP) == GLFW_PRESS) {
        pCamera->ProcessKeyBoard((float)deltaTime, EMovement::EUp);
    }
    if (glfwGetKey(window, GLFW_KEY_PAGE_DOWN) == GLFW_PRESS) {
        pCamera->ProcessKeyBoard((float)deltaTime, EMovement::EDown);
    }

    if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS) {
        int width, height;
        glfwGetWindowSize(window, &width, &height);
        pCamera->Set(width, height, pCamera->getStartPosition());
